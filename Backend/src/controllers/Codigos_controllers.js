...
// Obtener código por ID 
  ...
  try {
    ...
    const [codigos] = await connection.query(
      'SELECT id, codigo, status, descripcion, detalles, link_referencia, descripcion_sap, lead_time, dias_tolerancia, grupo_articulos, requestor_area ,tipo_bien FROM codigos WHERE id = ?',
      [id]
    );
