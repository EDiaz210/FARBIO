...
// Obtener código por ID 
  ...
  try {
    ...
    const [codigos] = await connection.query(
      'SELECT id, codigo, status, descripcion, detalles, link_referencia, descripcion_sap, lead_time, dias_tolerancia, cantidad_minima_pedido, unidad_compra, grupo_articulos, requestor_area ,tipo_bien, unidad_medida, nombre_solicitante FROM codigos WHERE id = ?',
      [id]
    );
