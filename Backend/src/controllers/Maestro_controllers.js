...
// UPDATE Y ENVIO A SAP - MAESTRO DE DATOS
  ...
  try {
    ...
    const codigoQuery = 'SELECT id, status FROM codigos WHERE id = ?';
    ...
    console.log('\nINICIANDO PROCESO MAESTRO DE DATOS');
    console.log(`Código ID: ${id}`);
    console.log(`Usuario: ${userName} (${userRole})`);
    ...
    console.log(`Item creado en SAP: ${sapItemResponse.data.ItemCode}`);
