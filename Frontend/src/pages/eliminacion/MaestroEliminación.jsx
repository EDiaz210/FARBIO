...
const MaestroEliminacion = ({ codigo, isOpen, onClose, onDeleted }) => {
  const [isDeleting, setIsDeleting] = useState(false);
  const { token } = storeAuth();
  const { fetchDataBackend } = useFetch();
  const claims = getAuthClaims(token);
  const userId = claims?.id;
  const userName = claims?.nombre || claims?.name || claims?.username || 'Maestro';
      ...
      <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl">
        <div className="mb-6 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">Esta acción no se puede deshacer. El código será eliminado permanentemente.</p>
          </div>
        </div>
...
export default MaestroEliminacion;
